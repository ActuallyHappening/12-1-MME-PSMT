...
fn main() {
	...
	let A_tl = trapezoidal_rule(d_tl, 0.0, 0.6, 12, precision).unwrap();
	...
	let A_m = trapezoidal_rule(f_middle, 0.0, 1.0, 20, precision).unwrap();
	let V_m = A_m * w * h * h_m;
	...
	let A_b = trapezoidal_rule(f_bottom, 0.0, 1.0, 20, precision).unwrap();
	let V_b = A_b * w * h * h_b;
